...
# ii. load data ----
load("pub data for gihub/oxs datasets for pub.Rdata", verbose = T)
...
# 4i. Mixed longitudinal > 35 past prime-----
# 4ia. m-l >35 OHDG -----
cross_frail_pub %>%
  ggplot(., aes(x = Age/365.25, y = OHDG_SG, group = sex, color = sex, shape = sex))  +
  geom_point() +
  geom_smooth(aes(linetype = sex), method = "lm") +
  scale_color_manual(values = cbPalette) +
  labs(x = "Age", y = "8-OHdG ng/mL", title = "8-OHdG Among M-L > 35 Sample (non-sig)") +
  theme_minimal() +
  theme(plot.title = element_text(hjust = 0.5, size = 18))

# 4ib. m-l >35 Isoprostanes ----
cross_frail_pub %>%
  ggplot(., aes(x = Age/365.25, y = Iso_SG, group = sex, color = sex))  +
  geom_point() +
  geom_smooth(method = "lm") +
  labs(x = "Age", y = "Isoprostane ng/mL") +
  ggtitle("Isoprostanes") +
  annotate("text", x = 37, y = 17, label = "A)", size = 8) +
  theme_minimal() +
  theme(plot.title = element_text(hjust = 0.5, size = 12)) 
...
cross_frail_pub %>%
  ...
  geom_point() +
  geom_smooth(method = "lm") +
  labs(x = "Age", y = "TBARS mM (time adjusted residual)") +
  ggtitle("MDA-TBARS") +
  annotate("text", x = 37, y = 35, label = "B)", size = 8) +
  theme_minimal() +
  theme(plot.title = element_text(hjust = 0.5, size = 12), legend.position = "none")

# 4id. m-l >35 Neopterin ----
cross_frail_pub %>%
  ggplot(., aes(x = Age/365.25, y = Neo_SG_TAR, group = sex, color = sex))  +
  geom_point() +
  geom_smooth(method = "lm") +
  labs(x = "Age", y = "Neopterin nM") +
  ggtitle("Neopterin Among M-L > 35 Sample (non-sig)") +
  theme(plot.title = element_text(hjust = 0.5, size = 12))
...
cross_frail_pub %>%
  ...
  geom_point() +
  geom_smooth(method = "lm") +
  labs(x = "Age", y = "TAC mM (time adjusted residual)") +
  ggtitle("TAC") +
  annotate("text", x = 37, y = 8, label = "C)", size = 8) +
  theme_minimal() +
  theme(plot.title = element_text(hjust = 0.5, size = 12)) 
